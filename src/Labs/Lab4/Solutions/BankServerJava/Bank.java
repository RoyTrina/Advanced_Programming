package Labs.Lab4.Solutions.BankServerJava;
    ...
    public int createAccount(int customerId) {
        synchronized (accounts) {
            ...
            for (Integer account : accounts.keySet())
                if (maxAccountNumber < account) maxAccountNumber = account;
    ...
    public void transfer(int customerId, int fromAccount, int toAccount, int amount) throws Exception {
        synchronized (accounts) {
            ...
            if (accounts.get(fromAccount).getBalance() < amount)
                throw new Exception("The balance of account " + fromAccount + " is " + accounts.get(fromAccount).getBalance() + " which is insufficient to transfer " + amount + ".");
            if (amount <= 0) throw new Exception("Transfer amount has to be a positive value.");
